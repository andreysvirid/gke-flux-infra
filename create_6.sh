...
TAG_FROM_VALUES=$(git show $SHA:kbot/values.yaml | grep 'tag:' | awk -F'"' '{print $2}') || echo "values.yaml не найден на этом коммите"
echo "Tag в values.yaml: $TAG_FROM_VALUES"
...
echo "✅ Образ в Pod: $IMAGE"

IMAGE_TAG=$(echo $IMAGE | awk -F':' '{print $2}')
if [[ "$IMAGE_TAG" == "$TAG_FROM_VALUES" ]]; then
    echo "✅ Тег образа соответствует values.yaml"
else
    echo "⚠️ Тег образа НЕ соответствует values.yaml!"
    echo "   Tag в values.yaml: $TAG_FROM_VALUES"
    echo "   Tag в Pod: $IMAGE_TAG"
fi
